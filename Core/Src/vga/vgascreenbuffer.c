...
void TIM1_CC_IRQHandler(void) {
    ...
    if (screenBuffer->outputState != VGAOutputActive) {
...
VGAError VGAStartOutput() {
    ...
    SET_BIT(screenBuf->bufferState.Bpp3.screenLineDMAStream->CR, DMA_SxCR_EN);

    UInt32 thresholdSelected = READ_BIT(screenBuf->bufferState.Bpp3.screenLineDMAStream->FCR, DMA_SxFCR_FTH);
    ...
    while (READ_BIT(screenBuf->bufferState.Bpp3.screenLineDMAStream->FCR, DMA_SxFCR_FS) != fifoStatusToReach) {
